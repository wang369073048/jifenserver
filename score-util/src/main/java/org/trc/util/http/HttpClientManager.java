...
public class HttpClientManager {
	...
	public static synchronized CloseableHttpClient getHttpClient() {
		if (httpClient == null) {
			...
			final HttpConnectionFactory<HttpRoute, ManagedHttpClientConnection> connFactory = new ManagedHttpClientConnectionFactory(DefaultHttpRequestWriterFactory.INSTANCE,
			...
			httpClient = HttpClients.custom()
					.setConnectionManager(manager)//指定HttpClient使用的连接池
					...
					.setRetryHandler(new DefaultHttpRequestRetryHandler(0, false)) // 设置重置次数，默认是3次；当前是禁用掉（根据需要开启）
					.build();
