...
public class TxJerseyTools {
     ...
     * 获取请求体
     */
    public static String requestBody(ContainerRequestContext requestContext) throws IOException {
     ...
     * 获取请求参数
     */
    public static Map<String, String> getParameter(ContainerRequestContext requestContext) throws IOException {
        Map<String, String> map = new HashMap<>();
        String requestStr = requestBody(requestContext);
        try {
            requestStr = URLDecoder.decode(requestStr, "UTF-8");
        } catch (Exception e) {
            //e.printStackTrace();
        }
        if (requestStr.startsWith("{")) {// json格式数据
            getJsonParams(requestStr, map);
        } else {
            String[] arrs = requestStr.split("&");
            for (String strs : arrs) {
                String[] str = strs.split("=", 2);
                if (str.length == 2) {
                    map.put(str[0], str[1]);
                }
            }
        ...
        // 获取地址栏数据
        getAddrParams(requestContext, map);
        // 获取路径数据
        getPathParams(requestContext, map);
        return map;
    }
